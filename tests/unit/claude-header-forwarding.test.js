...
describe("proxyAwareFetch — api.anthropic.com routing", () => {
  ...
  it("routes api.anthropic.com to native fetch (got-scraping disabled) and returns ok response", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      statusText: "OK",
  ...
  it("falls back to native fetch when the native path throws", async () => {
  ...
  it("does NOT add any special routing for non-Anthropic hosts", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      statusText: "OK",
    ...
    const res = await proxyAwareFetch("https://openrouter.ai/api/v1/chat/completions", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: "{}",
    });
